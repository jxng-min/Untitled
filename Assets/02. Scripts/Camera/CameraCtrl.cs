...
public class CameraCtrl : MonoBehaviour
{
    ...
    {
        if(!InventoryMain.Active && !EquipmentInventory.Active && !StatInventory.Active && !ConversationManager.Instance.IsTalking && !ItemShopManager.IsActive && !CraftingManager.IsActive)
        {
    ...
    private void Translation()
    {
        ...
        if(Physics.Raycast(m_player.position, ray_direction, out RaycastHit ray_info, m_camera_distance))
        {
            if(!ray_info.collider.CompareTag("Player") && !ray_info.collider.CompareTag("Enemy") && !ray_info.collider.CompareTag("Weapon") 
                && !ray_info.collider.CompareTag("Particle") && ray_info.collider != null)
            {
